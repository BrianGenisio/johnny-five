  ...
  ANALOG: {
    initialize: {
      value: function(opts, dataHandler) {
        var allRead, rawValues, state = priv.get(this);
        ...
        state.sensorStates = this.pins.map(function(pin) {
          ...
          sensorState.sensor.on("data", function(value) {
            ...
            if (allRead) {
              ...
              rawValues = state.sensorStates.map(function(sensorState) { return sensorState.rawValue; });
              
              dataHandler.call(this, rawValues);
            }
          });
  ...
  SEN_13582: {
    ...
    initialize: {
      value: function(opts, dataHandler) {
        ...
        this.io.i2cWrite(address, this.REGISTER.REG_RESET, 0x12);
        this.io.i2cWrite(address, this.REGISTER.REG_RESET, 0x34);
        
        this.io.i2cRead(address, this.REGISTER.REG_DATA_A, READLENGTH, function(data) {
          rawValues = [];
          
          for(var i=0; i<8; i++) {
            rawValues.push((data >> i) & 0x01);  
          }
...
function calibrationIsValid(calibration, raw) {
  return calibration &&
    (calibration.max && calibration.max.length === raw.length) &&
    (calibration.min && calibration.min.length === raw.length);
}
...
function maxLineValue() {
  return (this.raw.length - 1) * CALIBRATED_MAX_VALUE;
}
...
function ReflectanceArray(opts) {
  ...
  Object.defineProperties(this, {
    isOn: {
      get: function() {
        return this.emitterIsOn();
      }
    },
    isCalibrated: {
      get: function() {
        return calibrationIsValid(this.calibration, this.raw);
      }
    },
    isOnLine: {
      get: function() {
...
ReflectanceArray.prototype.enable = function() {
  this.enableEmitter(true);
 
  return this;
};

ReflectanceArray.prototype.disable = function() {
  this.enableEmitter(false);
...
ReflectanceArray.prototype.loadCalibration = function(calibration) {
  ...
  if (!calibrationIsValid(calibration, this.raw)) {
